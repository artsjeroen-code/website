(() => {
  ...
  const presets = {
    '4-7-8-0': { values: [4, 7, 8, 0], note: 'Diepe ontspanning — rustig patroon met lange uitademing; vaak gebruikt om tot rust te komen.' },
    '4-4-4-4': { values: [4, 4, 4, 4], note: 'Focus & kalmte — box breathing met vier gelijke fasen voor een strak, aandachtig ritme.' },
    '4-0-8-0': { values: [4, 0, 8, 0], note: 'Ontstressen — eenvoudige langzame ademhaling met extra nadruk op de uitademing.' },
    '5-0-5-0': { values: [5, 0, 5, 0], note: 'Rustig ritme — zes ademhalingen per minuut; gelijkmatig en geschikt voor rustige dagelijkse oefening.' },
    '7-0-11-0': { values: [7, 0, 11, 0], note: 'Diepe rust — zeer langzaam ritme met lange uitademing; kies dit alleen als het comfortabel voelt.' },
    '4-4-4-0': { values: [4, 4, 4, 0], note: 'Driehoek-focus — inademen, vasthouden en uitademen in drie gelijke delen, zonder slotpauze.' }
  };
  ...
  const releaseWakeLock = async () => {
    ...
    try { await activeWakeLock.release(); } catch (_) {}
  };
  ...
  const phaseTone = (key) => {
    const tones = { inhale: 440.0, holdFull: 493.88, exhale: 349.23, holdEmpty: 293.66 };
    playTone(tones[key] || 392.0, 0.34, 0.014);
  };

  const readSettings = () => {
    const settings = {
      inhale: clampInt(inhaleInput.value, 0, 20), holdFull: clampInt(holdFullInput.value, 0, 20),
      exhale: clampInt(exhaleInput.value, 0, 20), holdEmpty: clampInt(holdEmptyInput.value, 0, 20),
      repetitions: clampInt(repetitionsInput.value, 1, 99)
    };
    inhaleInput.value = settings.inhale; holdFullInput.value = settings.holdFull;
    exhaleInput.value = settings.exhale; holdEmptyInput.value = settings.holdEmpty;
  ...
  const syncPresetFromInputs = () => {
    ...
    if (presets[key]) { breathingPreset.value = key; presetNote.textContent = presets[key].note; }
    else { breathingPreset.value = 'custom'; presetNote.textContent = 'Eigen ritme — pas de vier fasen vrij aan tussen 0 en 20 seconden.'; }
  };

  const applyPreset = () => {
    const preset = presets[breathingPreset.value];
    if (!preset) { presetNote.textContent = 'Eigen ritme — pas de vier fasen vrij aan tussen 0 en 20 seconden.'; savePreferences(); return; }
    preset.values.forEach((value, index) => { phaseInputs[index].value = value; });
    presetNote.textContent = preset.note;
    updateSummary(); savePreferences();
  };
  ...
  const setInputsDisabled = (disabled) => { inputs.forEach((input) => { input.disabled = disabled; }); breathingPreset.disabled = disabled; };
  ...
  const renderRunningState = (now) => {
    ...
    if (elapsed >= session.totalSeconds) { finishSession(); return; }
    ...
    if (state === 'running' && token !== lastPhaseToken) { lastPhaseToken = token; phaseTone(currentPhase.key); }
  ...
  const tick = (now) => { if (state !== 'running') return; renderRunningState(now); if (state === 'running') animationFrame = requestAnimationFrame(tick); };

  const startSession = () => {
    const settings = readSettings();
    const cycleSeconds = getCycleSeconds(settings);
    if (cycleSeconds <= 0) { validationMessage.textContent = 'Kies minimaal één fase met een duur groter dan 0 seconden.'; return; }
    savePreferences(); ensureAudio(); session = buildSession(settings); state = 'running'; requestWakeLock();
    startedAt = performance.now(); pausedAt = 0; totalPaused = 0; lastPhaseToken = '';
    setInputsDisabled(true); startButton.disabled = true; pauseButton.disabled = false; pauseButton.textContent = 'Pauze';
    validationMessage.textContent = ''; repetitionStatus.textContent = `1 / ${settings.repetitions}`;
    remainingDuration.textContent = formatClock(session.totalSeconds);
    cancelAnimationFrame(animationFrame); renderRunningState(startedAt); animationFrame = requestAnimationFrame(tick);
  };

  const pauseSession = () => {
    if (state === 'running') {
      state = 'paused'; releaseWakeLock(); pausedAt = performance.now(); cancelAnimationFrame(animationFrame);
      renderRunningState(pausedAt); pauseButton.textContent = 'Hervat'; phaseLabel.textContent += ' — gepauzeerd'; return;
    }
    if (state === 'paused') {
      const resumedAt = performance.now(); totalPaused += resumedAt - pausedAt; pausedAt = 0; state = 'running'; requestWakeLock();
      pauseButton.textContent = 'Pauze'; animationFrame = requestAnimationFrame(tick);
    }
  };

  const finishSession = () => {
    cancelAnimationFrame(animationFrame); state = 'finished'; releaseWakeLock(); playTone(587.33, 0.52, 0.016); setBallScale(1);
    phaseLabel.textContent = 'Klaar'; phaseTime.textContent = '0,0 s'; remainingDuration.textContent = '00:00';
    repetitionStatus.textContent = `${session.settings.repetitions} / ${session.settings.repetitions}`;
    pauseButton.disabled = true; pauseButton.textContent = 'Pauze'; startButton.disabled = false; startButton.textContent = 'Opnieuw'; setInputsDisabled(false);
  };

  const resetSession = () => {
    cancelAnimationFrame(animationFrame); state = 'idle'; releaseWakeLock(); session = null; startedAt = 0; pausedAt = 0; totalPaused = 0; lastPhaseToken = '';
    setInputsDisabled(false); setBallScale(1); phaseLabel.textContent = 'Klaar'; phaseTime.textContent = '0,0 s';
    startButton.disabled = false; startButton.textContent = 'Start'; pauseButton.disabled = true; pauseButton.textContent = 'Pauze';
    syncPresetFromInputs(); updateSummary(); savePreferences();
  };
  ...
  inputs.forEach((input) => input.addEventListener('input', () => {
    if (state === 'idle' || state === 'finished') { if (phaseInputs.includes(input)) syncPresetFromInputs(); updateSummary(); savePreferences(); }
  }));
  document.addEventListener('visibilitychange', () => { if (document.visibilityState === 'visible' && state === 'running') requestWakeLock(); });

  restorePreferences(); syncPresetFromInputs(); updateSummary();
})();
